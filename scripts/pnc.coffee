...
module.exports = (robot) ->
  ...
  check_if_server_online = (url, server_name, server_type, res) ->
    ...
    Rest.get(url).on('success', (result) ->
      res.send first_str + "Online".irc.white.bold()
    ).on('fail', (result) ->
      res.send first_str + "Online, but throwing errors".irc.yellow.bold()
    ).on('error', (result) ->
  ...
  robot.respond /(.*)running builds(.*)/i, (res) ->
    Rest.get(running_builds_url).on('success', (result) ->
      if result && result.content && result.content.length > 0
        ...
        res.reply "No running builds on #{config.pnc_main_server.irc.white()}"
    ).on('fail', (result) ->
      res.reply "#{config.pnc_main_server.irc.white()} is throwing errors"
    ).on('error', (result) ->
      res.reply "#{config.pnc_main_server.irc.white()} is " + "DOWN".irc.red.bold.bgwhite()
    )
